...
GASWR_AVX_LIBRARIES = $(MESA_BUILD_PATH)/gallium-aux.lib
...
GASWR_AVX2_LIBRARIES = $(MESA_BUILD_PATH)/gallium-aux.lib
...
all: $(MESA_BUILD_PATH) $(MESA_BUILD_PATH)/util.lib $(MESA_BUILD_PATH)/compiler.lib \
	 ...
	 $(MESA_BUILD_PATH)/gallium-svga.lib $(MESA_BUILD_PATH)/gallium-swr.lib \
	 $(MESA_BUILD_PATH)/gallium-swr-avx.dll $(MESA_BUILD_PATH)/gallium-swr-avx2.dll \
...
install: all
	...
	cp -rf include/EGL/ $(VALI_APPLICATION_PATH)/include/
	cp -rf include/GL/*.h $(VALI_APPLICATION_PATH)/include/GL
	cp -rf include/GLES/ $(VALI_APPLICATION_PATH)/include/
	cp -rf include/GLES2/ $(VALI_APPLICATION_PATH)/include/
	cp -rf include/GLES3/ $(VALI_APPLICATION_PATH)/include/
	cp -rf include/KHR/ $(VALI_APPLICATION_PATH)/include/
	cp -rf include/vulkan/ $(VALI_APPLICATION_PATH)/include/
...
src/gallium/drivers/swr/rasterizer/jitter/gen_builder.hpp: src/gallium/drivers/swr/rasterizer/codegen/gen_llvm_ir_macros.py
	python $< --input $(VALI_APPLICATION_PATH)/include/llvm/IR/IRBuilder.h --output src/gallium/drivers/swr/rasterizer/jitter --gen_h
...
$(MESA_BUILD_PATH)/gallium-swr-avx2.dll: $(GASWR_SOURCES_GEN_H) $(GASWR_SOURCES_GEN_S) $(GASWR_SOURCES_GEN_C) $(GASWR_SOURCES_GEN_CXX) $(GASWR_AVX2_OBJECTS_S) $(GASWR_AVX2_OBJECTS_C) $(GASWR_AVX2_OBJECTS_CXX)
	@printf "%b" "\033[0;36mCreating shared library " $@ "\033[m\n"
